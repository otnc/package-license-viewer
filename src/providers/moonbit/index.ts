...
export class MoonbitLicenseProvider implements LicenseProvider {
  ...
  private readonly lookup = new MooncakesLookup();
  private readonly index = new RegistryIndex();
  ...
  parse(document: vscode.TextDocument): MoonbitEntry[] {
    const format = manifestFormat(document.uri.path);
    if (!format) return [];
    return parseManifest(document.getText(), document.uri.toString(), format).entries;
  }
