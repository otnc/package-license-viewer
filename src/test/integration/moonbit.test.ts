...
import { MoonbitLicenseProvider } from "../../providers/moonbit";
...
suite("MoonBit workspace", () => {
  ...
  test("dispatches a plaintext moon.mod and resolves it without touching mooncakes.io", async () => {
    ...
    try {
      ...
      const unpacked = await provider.resolve(entries[1], document, noCancel);
      ...
      const member = await provider.resolve(entries[2], document, noCancel);
  ...
  test("reads the legacy JSON manifest and leaves path and git dependencies alone", async () => {
    ...
    try {
      ...
      const provider = findProvider(createProviders(cache), document);
      assert.ok(provider instanceof MoonbitLicenseProvider);
      const entries = provider.parse(document);
      assert.deepEqual(
      ...
      assert.deepEqual(await provider.resolve(entries[2], document, noCancel), {
        source: "skipped",
        detail: "local path dependency",
      });
      assert.deepEqual(await provider.resolve(entries[3], document, noCancel), {
        source: "skipped",
        detail: "git dependency",
      });
      // Nothing on disk knows this module, and requests are turned off in this workspace
      const pinned = await provider.resolve(entries[1], document, noCancel);
